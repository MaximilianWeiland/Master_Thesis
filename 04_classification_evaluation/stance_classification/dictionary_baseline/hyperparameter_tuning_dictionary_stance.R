...
# store these metrics in a json file and export
optimal_configs <- list(
  window_size = best_combination$window_size,
  positive_threshold = best_combination$pos_threshold,
  negative_threshold = best_combination$neg_threshold,
  macro_f1_score = round(best_macro_f1, 4)
)
configs_json <- toJSON(optimal_configs)
...
write(configs_json, file = output_path)