...
with open(data_root / "ht_results/ht_bert_sequence_stance.json", "r") as f:
...
average_metrics = cv_stance(model_names=model_names, training_data=data, dataset_class=StanceDataset, label2id=label_to_id,
                            id2label=id_to_label, num_folds=5, optimal_configurations=hyperparameter_tuning_results, device=device, seed=2)

# export the cross-validation metrics
with open(data_root / "cv_results/cv_metrics_bert_stance_sequence.json", "w") as f:
