...
class ModelMaskHN(nn.Module):
    ...
    def encode(self, input_ids, attention_mask):
        """Return normalized projected embeddings"""
        outputs = self.encoder(input_ids=input_ids, attention_mask=attention_mask)
        h = self.extract_mask_embedding(input_ids, outputs.last_hidden_state)
        z = self.projector(h)
        z = F.normalize(z, p=2, dim=1)
...
def train_triplet(model, dataloader, loss_fn, optimizer, device):
    ...
    for batch in progress_bar:
        ...
        _, anchor_emb = model.encode(batch["anchor_input_ids"], batch["anchor_attention_mask"])
        _, pos_emb = model.encode(batch["positive_input_ids"], batch["positive_attention_mask"])
        _, neg_emb = model.encode(batch["negative_input_ids"], batch["negative_attention_mask"])
